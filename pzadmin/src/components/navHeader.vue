...
<template>
    <div class="elHeader-container">
        <div class="nav-left flex-box">
            <el-icon style="margin-right: 10px" size="26" @click="menuStore.collapseMenu"><Expand /></el-icon>
            <ul class="flex-box">
                <li
                ...
                >
                    ...
                    <!-- 使用编程式导航 router.push   -->
                    <a class="text" @click="routerPush(item.path, item.activa)">{{ item.name }}</a>
                    <el-icon size="20" class="close" @click="closeTab(item, index)"><Close /></el-icon>
                </li>
            </ul>
        </div>
        <div class="nav-right">
