<script setup>
    ...
    import { userManageList, menuSelectlist, updateUser } from '../../../api/index.js';
    ...
    const open = (row = {}) => {
        ...
        Object.assign(formData, { mobile: row.mobile, name: row.name, permissions_id: row.permissions_id });
    };
    ...
    const rules = reactive({
        ...
        permissions_id: [{ required: true, message: '权限ID不得为空', trigger: 'blur' }],
    });
</script>
...
<template>
    <panelHead></panelHead>
    <!-- 数据表格 -->
    <el-table v-loading="tableLoading" :data="userList.list" height="530" style="width: 100%">
        ...
        <el-table-column label="操作工具" width="160" align="center">
            <template #default="scope">
    ...
    <!-- 对话框 -->
    <el-dialog v-model="dialogFormVisible" :before-close="beforeClose" title="账号管理" width="500">
        <el-form :model="formData" :rules="rules" ref="formRef" style="max-width: 360px; margin: 0 auto">
            ...
            <el-form-item prop="name" size="large">
                <el-input v-model="formData.name" prefix-icon="User" placeholder="请输入昵称" />
            </el-form-item>
            <el-form-item prop="permissions_id" size="large">
                <el-select v-model="formData.permissions_id" suffix-icon="ArrowDown" placeholder="请选择权限">
                    <el-option v-for="(item, index) in userList.options" :key="index" :label="item.name" :value="item.id" />
                </el-select>
            </el-form-item>
        </el-form>
        <template #footer>
