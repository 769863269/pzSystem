    ...
    const onsubmit = async (formEl) => {
        ...
        await formEl.validate((valid, fields) => {
            if (valid) {
                ...
                userSetmenu({ name: formData.name, permissions: formData.permissions, id: formData.id }).then((res) => {
                    if (res.data.code === 10000) {
                        ElMessage({
                            message: '提交数据成功',
                            type: 'success',
...
<template>
    <!-- 对话框 -->
    <el-dialog v-model="dialogFormVisible" :before-close="beforeClose" title="菜单管理" width="500">
        <el-form :model="formData" :rules="rules" ref="formRef" style="max-width: 360px; margin: 0 auto">
            ...
            <el-form-item prop="name" size="large">
                <el-input v-model="formData.name" prefix-icon="EditPen" placeholder="请填写权限名称 *必填" />
            </el-form-item>
            <el-form-item>
    ...
    <!-- 数据表格 -->
    <el-table v-loading="tableLoading" :data="tableData.list" height="530" style="width: 100%; margin-top: 20px">
        <el-table-column prop="id" label="编号" width="160" />
        <el-table-column prop="name" label="昵称" width="160" />
        <el-table-column prop="permissionName" label="权限规则" flex="1" min-width="500" />
